...
interface FormationDesignerProps {
  ...
  players?: Player[];
  initialData?: any;
  onPlayerClick?: (player: Player, role: string) => void;
  hidePitch?: boolean;
  dbFormation?: any;
}
...
const FormationDesigner = React.forwardRef<FormationDesignerHandle, FormationDesignerProps>(({
  ...
  const [selected, setSelected] = useState(initialData?.template || dbFormation?.template || formationKey);
  const formation = dbFormation || FORMATIONS[selected] || FORMATIONS["4-4-2"];
  ...
  const [assignments, setAssignments] = useState<Record<number, Player>>(() => {
    const initial: Record<number, Player> = {};
    const sourcePositions = initialData?.positions || dbFormation?.positions;
    if (sourcePositions) {
      sourcePositions.forEach((p: any, idx: number) => {
        if (p.playerId) {
          const player = players.find(pl => String(pl.id) === String(p.playerId));
          if (player) initial[idx] = player;
        }
      });
    }
    return initial;
  ...
  useEffect(() => {
    ...
    if (sourcePositions) {
      ...
      const assign: any = {};
      sourcePositions.forEach((p: any, idx: number) => {
        custom[`${(initialData?.template || dbFormation?.template) || selected}-${idx}`] = { x: p.x, y: p.y };
        if (p.playerId) {
          const player = players.find(pl => String(pl.id) === String(p.playerId));
          if (player) assign[idx] = player;
        }
      });
      setCustomPositions(custom);
      setAssignments(assign);
    } else if (initialData?.template || dbFormation?.template) {
      setSelected(initialData?.template || dbFormation?.template);
    }
  }, [initialData, dbFormation, players]);
  ...
  const handlePointerUp = useCallback((e: React.PointerEvent, idx: number) => {
    if (dragRef.current?.idx === idx) {
      ...
      if (dx < 5 && dy < 5) {
        ...
        if (assigned && onPlayerClick) {
          onPlayerClick(assigned, formation.positions[idx].role);
  ...
  return (
    <div className={hidePitch ? "" : "space-y-4"}>
      {!hidePitch && (
      ...
      <div
        ref={pitchRef}
        className={hidePitch ? "absolute inset-0 select-none" : "relative w-full overflow-hidden rounded-xl border border-border select-none"}
        style={hidePitch ? {} : { aspectRatio: "3/4" }}
      >
        {!hidePitch && (
          <>
            {/* Grass background */}
        ...
        {/* Player positions (overlay on pitch) */}
        <div className="absolute inset-0 pointer-events-none">
          {/* Grass stripe effect */}
          {!hidePitch && [0, 1, 2, 3, 4, 5, 6, 7].map((i) => (
            <div
              key={i}
