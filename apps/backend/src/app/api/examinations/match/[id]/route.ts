...
import { matchAnalysisUpdateSchema } from "@/lib/validation";
...
export async function GET(request: Request, { params }: { params: { id: string } | Promise<{ id: string }> }) {
    ...
    try {
        ...
        const item = await prisma.matchAnalysis.findUnique({
            where: { id },
             include: { match: true }
        });
